...
class BertClassifier(nn.Module):
    def __init__(self, freeze_bert=True):
        ...
        # Specify hidden size of BERT, hidden size of our classifier, and
        # number of labels
...
def train(
    ...
    for epoch_i in range(epochs):
        ...
        print(f"{'Epoch':^7} | {'Batch':^7} | {'Train Loss':^12} |\
             {'Val Loss':^10} | {'Val Acc':^9} | {'Elapsed':^9}")
        print("-" * 70)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # Clip the norm of the gradients to 1.0 to prevent "exploding
            # gradients"
            ...
            if (step % 20 == 0 and step != 0) or (
                ...
                print(
                    f"{epoch_i + 1:^7} | {step:^7} | \
                        {batch_loss / batch_counts:^12.6f}\
                         | {'-':^10} | {'-':^9} | {time_elapsed:^9.2f}")
        ...
        if evaluation:
            ...
            print(f"{epoch_i + 1:^7} | {'-':^7} | {avg_train_loss:^12.6f} | \
                {val_loss:^10.6f} | {val_accuracy:^9.2f} \
                    | {time_elapsed:^9.2f}")
            print("-" * 70)
        print("\n")
